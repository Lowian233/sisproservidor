...
class AjaxController extends Controller
{
	...
	public function SGenerRespel(Request $request, $id)
	{
		if ($request->ajax()) {
			...
			// Validar que la sede del generador existe
			if (!$SGener) {
			...
			// Obtener IDs de residuos ya asignados a esta sede de generador (optimizado)
			$ResSGenersIds = DB::table('residuos_geners')
				->where('FK_SGener', '=', $SGener->ID_GSede)
				->where('DeleteSGenerRes', '=', 0)
				->pluck('FK_Respel')
				->toArray();

			// Obtener el cliente asociado al generador
			...
			// Construir la consulta de residuos
			...
			// Excluir residuos ya asignados (optimizado con whereNotIn)
			if (!empty($ResSGenersIds)) {
				$query->whereNotIn('respels.ID_Respel', $ResSGenersIds);
			}

			// Filtrar por cliente según el rol del usuario
			if (in_array(Auth::user()->UsRol, Permisos::TODOPROSARC)) {
				// Usuarios de PROSARC pueden ver residuos del cliente asociado al generador
				$query->where('clientes.ID_Cli', $Cliente->ID_Cli);
			} elseif (Auth::user()->UsRol === 'Programador') {
				$query->where('clientes.ID_Cli', $Cliente->ID_Cli);
	...
	public function RespelGener(Request $request, $slug){
		if ($request->ajax()){
			try {
				// Validar que el slug de la sede del generador existe
				$SGener = GenerSede::where('GSedeSlug', $slug)->first();
				if (!$SGener) {
					return response()->json([]);
				}
				...
				$Respels = DB::table('residuos_geners')
					...
					->select('residuos_geners.SlugSGenerRes', 'respels.RespelName', 'respels.RespelSlug', 'respels.ID_Respel', 'respels.SustanciaControlada', 'respels.YRespelClasf4741','respels.ARespelClasf4741','requerimientos.FK_ReqTrata', 'requerimientos.forevaluation', 'requerimientos.ofertado')
					->whereIn('respels.RespelStatus', ['Aprobado', 'Revisado', 'Falta TDE', 'TDE actualizada', 'Vencido'])
					->where('respels.RespelDelete', 0)
				...
				foreach ($Respels as $key => $value) {
					if (isset($Respels[$key]->FK_ReqTrata) && $Respels[$key]->FK_ReqTrata && $Respels[$key]->ofertado == 1) {
				...
				Log::error('Error en RespelGener: ' . $e->getMessage(), [
					'slug' => $slug,
					'trace' => $e->getTraceAsString()
				]);
				return response()->json(['error' => 'Error al cargar residuos'], 500);
			}
		}
	}
	...
	public function certificarServicio(Request $request, $servicio)
	{
		if ($request->ajax()) {
			if (in_array(Auth::user()->UsRol, Permisos::SolSerCertifi) || in_array(Auth::user()->UsRol2, Permisos::SolSerCertifi)) {
				...
				if ($resCode == 200) {
					...
					// Envío de correo: NO debe tumbar la certificación si falla
					try {
						...
						$regularComercial = in_array($Solicitud->SolSerTipo, ['Cliente', 'Externo', 'Generador'], true);
						$destinatarios = [];
						if (! $regularComercial) {
							$comercial = null;
							if ($email && !empty($email->CliComercial)) {
								$comercial = Personal::where('ID_Pers', $email->CliComercial)->first();
							}
							if ($comercial && !empty($comercial->PersEmail)) {
								$destinatarios[] = $comercial->PersEmail;
							}

							if ($Solicitud->SolServMailCopia !== "null" && $Solicitud->SolServMailCopia !== null && $Solicitud->SolServMailCopia !== "") {
								foreach ((array) json_decode($Solicitud->SolServMailCopia, true) as $value) {
									if (!empty($value)) {
										$destinatarios[] = $value;
									}
								}
						...
						// Si no hay email de contacto, no intentamos enviar
						if ($email && !empty($email->PersEmail)) {
							Mail::to($email->PersEmail)
								->cc($destinatarios)
								->send(new SolSerEmail($email));
						}
					} catch (\Throwable $e) {
						Log::error('Fallo enviando correo de certificación', [
							'slug' => $Solicitud->SolSerSlug,
							'ID_SolSer' => $Solicitud->ID_SolSer,
							'error' => $e->getMessage(),
						]);
						// Seguimos: el servicio ya quedó certificado y registrado en auditoría/observación
					}
				}
		...
		// Si alguien pega la URL en el navegador (no AJAX), respondemos controlado
		return response()->json(['message' => 'Solicitud inválida', 'code' => 400], 400);
	}
    ...
    public function sendRecordatorio(Request $request)
    {
        ...
        if ($Solicitud->SolSerStatus != 'Recepcionado') {
			abort(
				response()->json([
						'message' => 'El servicio NO está en status (RECEPCIONADO)',
						'newtoken' => csrf_token()
				], 403)
			);
		}
