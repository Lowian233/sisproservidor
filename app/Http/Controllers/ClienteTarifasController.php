...
class ClienteTarifasController extends Controller
{
     ...
     */
    public function create($CliSlug, Request $request)
    {
        $cliente = Cliente::where('CliSlug', $CliSlug)->with(['clientetarifa.rangos', 'clientetarifa.tratamiento', 'sedes'])->first();
        
        if (!$cliente) {
            abort(404);
        }
        
        // Obtener todos los tratamientos de las sedes del proveedor
        $sedeIds = $cliente->sedes->pluck('ID_Sede')->toArray();
        $tratamientos = Tratamiento::whereIn('FK_TratProv', $sedeIds)
            ->where('TratDelete', 0)
            ->get();
        
        // Si viene un tratamiento específico en la URL, pre-seleccionarlo
        $tratamientoSeleccionado = $request->query('tratamiento', null);

        return view('cliente_tarifas.create', compact(['cliente', 'tratamientos', 'tratamientoSeleccionado']));
    }
...
*    * @param  string  $CliSlug
    ...
    public function store(Request $request, $CliSlug)
    {
        ...
        return redirect()->route('clientetarifas.create', ['slug' => $cliente->CliSlug]);
    }
    ...
    public function destroy($CliSlug, $ID_CRango)
    {
        ...
        return redirect()->route('clientetarifas.create', ['slug' => $CliSlug]);
    }
}
