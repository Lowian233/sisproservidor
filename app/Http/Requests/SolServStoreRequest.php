...
class SolServStoreRequest extends FormRequest
{
    ...
    {
        // Verificar si es usuario interno de Prosarc (solicitud interna)
        $esUsuarioInterno = in_array(Auth::user()->UsRol, Permisos::PUEDE_SOLICITAR_PLANTA);
        
        // Determinar el ID del cliente para la validación
        // Para usuarios internos, usar Prosarc (ID 1), de lo contrario usar el cliente del usuario
        $clienteIdUsuario = userController::IDClienteSegunUsuario();
        $clienteId = $esUsuarioInterno ? 1 : ($clienteIdUsuario ?? 1);
        
        $rules = [
            'FK_SolSerPersona'  => ['required',Rule::exists('personals', 'PersSlug')->where(function ($query) use ($request, $clienteId){
        ...
        switch ($request->input('SolSerTipo')) {
            case 96:
                // Para usuarios internos, los campos de transportador son opcionales
                if ($esUsuarioInterno) {
                    $rules = [
                        'SolSerNameTrans'    => 'nullable|max:255',
                        'SolSerNitTrans'     => 'nullable|max:20',
                        'SolSerAdressTrans'  => 'nullable|max:255',
                        'SolSerCityTrans'    => ['nullable',Rule::exists('municipios', 'ID_Mun')],
                        'SolSerConductor'    => 'nullable|array',
                        'SolSerConductor.*'  => 'nullable|string|max:255',
                        'SolSerVehiculo'     => 'nullable|array',
                        'SolSerVehiculo.*'   => 'nullable|string|max:255',
                    ];
                } else {
                    $rules = [
            ...
            case 97:
            case 98:
                // Para usuarios internos, el transportador es opcional
                if ($esUsuarioInterno) {
                    $rules = [
                        'SolSerTransportador' => 'nullable',
                        'SolSerConductor'    => 'nullable|array',
                        'SolSerConductor.*'  => 'nullable|string|max:255',
                        'SolSerVehiculo'     => 'nullable|array',
                        'SolSerVehiculo.*'   => 'nullable|string|max:255',
        ...
        foreach ($request->input('SGenerador') as $Generador => $value) {
            ...
            if(isset($request['FK_SolResRg'][$Generador][0])){
                for ($y=0; $y < count($request['FK_SolResRg'][$Generador]); $y++) {
                    ...
                    $rules['SolResCantEmbalaje.'.$Generador.'.'.$y] = 'required|numeric|min:1';
      ...
      /*  foreach ($this->request->get('SGenerador') as $Generador => $value) {
            ...
            if (isset($this->instance()->all()['FK_SolResRg'][$Generador])) {
                for ($y=0; $y < count($this->instance()->all()['FK_SolResRg'][$Generador]); $y++) {
                    ...
                    $attributes['SolResCantEmbalaje.'.$Generador.'.'.$y] = '"Cantidad de embalaje" del residuo (N°'.($y+1).') del generador (N° '.($Generador+1).')';
