		...
		<div class="col-md-16 col-md-offset-0">
			<div class="box">
				<div class="box-header">
					...
					@endphp
					<td>
						<a href="/solicitud-servicio/{{$programacion->servicio->SolSerSlug}}/recibomaterial" target="_blank" class="btn btn-primary">Recibo Material</a>
					</td>
					<td>
						...
						<form action="/vehicle-programacion/{{$programacion->FK_ProgServi}}/añadirVehiculo" method="POST" id="formularioCreate" data-toggle="validator">
							...
							<div class="box-body">
								...
								<div class="form-group col-xs-12 col-md-12 ambos" hidden="true">
									...
									<select name="FK_ProgAyudante" id="modalFK_ProgAyudante" class="form-control">
										<option value="">{{ __('adminlte::message.select') }}</option>
										@foreach($ayudantes as $ayudante)
...
$(document).ready(function() {
    ...
    $('#formularioTransportador').submit(function(e) {
        var $form = $(this);
        var tipo = $form.find('#ProgVehtipo').val();
        var isValid = true;

        if (tipo == '1') {
            // Prosarc - solo validar que tenga vehículo y conductor
            if (!$form.find('#FK_ProgVehiculo').val() || !$form.find('#FK_ProgConductor').val()) {
                alert('Por favor complete vehículo y conductor para transportador Prosarc');
                isValid = false;
            }
        } else if (tipo == '2') {
            // Alquilado: validar transportador, conductor y placa
            var transportador = ($form.find('[name="transport"]').val() || '').trim();
            var conductor = ($form.find('[name="ProgVehNameConductorEXT"]').val() || '').trim();
            var placa = ($form.find('[name="ProgVehPlacaEXT"]').val() || '').trim();

            if (!transportador || !conductor || !placa) {
                alert('Por favor complete transportador, conductor y placa para transportador alquilado');
                isValid = false;
            }
        }
...
<script>
	...
	window.TranspotadorProsarc = function(){
		...
		$('#ProgVehNameConductorEXT').attr('required', false);
		$('#ProgVehPlacaEXT').attr('required', false);
	...
	window.TranspotadorAlquilado = function() {
		...
		$('#ProgVehNameConductorEXT').attr('required', true);
		$('#ProgVehPlacaEXT').attr('required', true);
