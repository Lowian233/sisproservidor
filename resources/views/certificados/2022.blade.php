									...
									@switch($certificado->CertType)
										@case(0)
											@if($certificado->CertSrc!=="CertificadoDefault.pdf")
												@php
													$_yCertPath = storage_path('app/public/certificadoRegular/'.$certificado->CertSrc);
													$_yCertImg = public_path('img/Certificados/'.$certificado->CertSrc);
												@endphp
												<td class="text-center"><a method='get' href='{{ file_exists($_yCertPath) ? asset('storage/certificadoRegular/'.$certificado->CertSrc).'?v='.filemtime($_yCertPath) : asset('img/Certificados/'.$certificado->CertSrc).'?v='.(file_exists($_yCertImg) ? filemtime($_yCertImg) : time()) }}' target='_blank' class='btn btn-success'><i class='fas fa-file-contract fa-lg'></a></td>
											@else
										...
										@case(1)
											@if($certificado->CertSrcManif!=="CertificadoDefault.pdf")
												@php
													$_yManifName = ($certificado->CertSrc !== 'CertificadoDefault.pdf') ? $certificado->CertSrc : $certificado->CertSrcManif;
													$_yManifPath = storage_path('app/public/manifiestosRegular/'.$_yManifName);
													$_yManifImg = public_path('img/Manifiestos/'.$_yManifName);
												@endphp
												<td class="text-center"><a method='get' href='{{ file_exists($_yManifPath) ? asset('storage/manifiestosRegular/'.$_yManifName).'?v='.filemtime($_yManifPath) : asset('img/Manifiestos/'.$_yManifName).'?v='.(file_exists($_yManifImg) ? filemtime($_yManifImg) : time()) }}' target='_blank' class='btn btn-primary'><i class='far fa-file-alt fa-lg'></a></td>
											@else
										...
										@case(2)
											@if($certificado->CertSrcExt!=="CertificadoDefault.pdf")
												@php($_yM2Path = storage_path('app/public/manifiestosRegular/'.$certificado->CertSrc))
												<td class="text-center"><a method='get' href="{{ asset('storage/manifiestosRegular/'.$certificado->CertSrc.'') }}?v={{ file_exists($_yM2Path) ? filemtime($_yM2Path) : time() }}"  target='_blank' class='btn btn-warning'><i class='far fa-file-alt fa-lg'></a></td>
											@else
												<td class="text-center"><a disabled method='get' href='/img/CertificadoDefault.pdf' target='_blank' class='btn btn-default'><i class='far fa-file-alt fa-lg'></a></td>
											@endif
											@break
...
function firmarDocumento(CertSlug){
	...
	$.ajax({
		url: "/firmarCertificado/"+CertSlug,
		method: 'PUT',
		data:{},
		beforeSend: function(){
