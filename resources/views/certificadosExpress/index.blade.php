						...
						<table class="table table-compact table-bordered table-striped">
							...
							<tbody>
								...
								<tr>
									...
									@switch($certificado->CertType)
										@case(0)
											@if($certificado->CertSrc!=="CertificadoDefault.pdf")
												@php($_expressCertPath = storage_path('app/public/certificadoExpress/E-'.sprintf('%07s', $certificado->ID_Cert).'.pdf'))
												<td class="text-center"><a method='get' href='{{ asset('storage/certificadoExpress/E-'.sprintf('%07s', $certificado->ID_Cert).'.pdf').'?v='.(file_exists($_expressCertPath) ? filemtime($_expressCertPath) : time()) }}' target='_blank' class='btn btn-success'><i class='fas fa-file-contract fa-lg'></a></td>
											@else
										...
										@case(1)
											@if($certificado->CertSrcManif!=="CertificadoDefault.pdf")
												@php($_exManifName = $certificado->CertSrc !== 'CertificadoDefault.pdf' ? $certificado->CertSrc : $certificado->CertSrcManif)
												@php($_exManifPath = storage_path('app/public/manifiestosExpress/'.$_exManifName))
												@php($_exManifImgPath = public_path('img/Manifiestos/'.$_exManifName))
												<td class="text-center"><a method='get' href='{{ file_exists($_exManifPath) ? asset('storage/manifiestosExpress/'.$_exManifName).'?v='.filemtime($_exManifPath) : asset('img/Manifiestos/'.$_exManifName).'?v='.(file_exists($_exManifImgPath) ? filemtime($_exManifImgPath) : time()) }}' target='_blank' class='btn btn-primary'><i class='far fa-file-alt fa-lg'></a></td>
											@else
										...
										@case(2)
											@if($certificado->CertSrcExt!=="CertificadoDefault.pdf")
												@php($_extImgPath = public_path('img/CertificadosEXT/'.$certificado->CertSrcExt))
												<td class="text-center"><a method='get' href='/img/CertificadosEXT/{{$certificado->CertSrcExt}}?v={{ file_exists($_extImgPath) ? filemtime($_extImgPath) : time() }}' target='_blank' class='btn btn-warning'><i class='far fa-file-alt fa-lg'></a></td>
											@else
												<td class="text-center"><a disabled method='get' href='/img/CertificadoDefault.pdf' target='_blank' class='btn btn-default'><i class='far fa-file-alt fa-lg'></a></td>
											@endif
											@break
									...
									@endif
									@php($Status = ['Conciliado', 'Tratado', 'Facturado'])
									@php($Status2 = ['Conciliado', 'Tratado'])
									@if(in_array(Auth::user()->UsRol, Permisos::SIGNMANIFCERT))
