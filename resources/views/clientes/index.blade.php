					...
					<div class="box-body">
						<table id="clientesTable" class="table table-compact table-bordered table-striped">
							...
							<tr style="{{$cliente->CliDelete === 1 ? 'color: red;' : ''}}">
								...
								<td>
									@if(in_array(Auth::user()->UsRol, Permisos::TODOPROSARC))
									<form action="{{ route('clientes.toggleProveedor', $cliente->CliSlug) }}" method="POST" style="display:inline-block; margin-bottom: 5px;">
										@csrf
										@method('PUT')
										<button type="submit" class="btn btn-xs {{ ($cliente->provedor ?? 0) ? 'btn-warning' : 'btn-default' }}" title="{{ ($cliente->provedor ?? 0) ? 'Quitar proveedor' : 'Marcar como proveedor' }}">
											<i class="fas fa-truck"></i> {{ ($cliente->provedor ?? 0) ? 'Es Proveedor' : 'No Proveedor' }}
										</button>
									</form>
									<br>
									@endif
