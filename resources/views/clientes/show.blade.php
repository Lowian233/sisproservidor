...
<div class="container-fluid spark-screen">
	...
	<div class="row">
		<div class="col-md-4">
			<div class="box {{ ($cliente->CliActivo ?? 1) == 0 ? 'box-default' : 'box-info' }}">
				<div class="box-body box-profile">
					<div class="col-md-12 col-xs-12">
						{{-- @if(in_array(Auth::user()->UsRol, Permisos::PROGRAMADOR) && Route::currentRouteName() !== 'cliente-show')
							...
							@endif
						@endif --}}
						@if(in_array(Auth::user()->UsRol, Permisos::JefeComercial) || in_array(Auth::user()->UsRol2, Permisos::JefeComercial) || in_array(Auth::user()->UsRol, Permisos::COMERCIALEINGRURNO) || in_array(Auth::user()->UsRol2, Permisos::COMERCIALEINGRURNO))
							@if(($cliente->CliActivo ?? 1) == 0)
								<form action="{{ route('crm.clientes.toggle-activo', $cliente->CliSlug) }}" method="post" style="display: inline;">
									...
									<button type="submit" class="btn btn-success pull-right" title="Activar cliente (volverá a contar en totales CRM)"><i class="fas fa-check-circle"></i> Activar</button>
								</form>
							@else
								<form action="{{ route('crm.clientes.toggle-activo', $cliente->CliSlug) }}" method="post" style="display: inline;" onsubmit="return confirm('¿Desactivar este cliente? Ya no contará en el total del CRM hasta que lo reactive.');">
									@csrf
									<input type="hidden" name="activo" value="0">
									<input type="hidden" name="redirect" value="cliente">
									<button type="submit" class="btn btn-warning pull-right" title="Desactivar (no contará en totales CRM)"><i class="fas fa-pause-circle"></i> Desactivar</button>
								</form>
							@endif
						@endif
						@if(($cliente->CliActivo ?? 1) == 0)
							<span class="label label-warning pull-right" style="margin-right: 5px;">Desactivado (CRM)</span>
						@endif
						...
						@if(in_array(Auth::user()->UsRol, Permisos::SOLSERACEPTADO))
							...
							@endif
						@endif
						{{-- Botón para crear solicitud de servicio --}}
						@if(in_array(Auth::user()->UsRol, Permisos::COMERCIALEINGRURNO) || in_array(Auth::user()->UsRol2, Permisos::COMERCIALEINGRURNO))
							<a href="{{ route('solicitud-servicio.create') }}?ID_Cli={{ $cliente->ID_Cli }}" class="btn btn-primary pull-right" style="margin-left: 5px;" title="Crear solicitud de servicio para este cliente">
								<i class="fas fa-file-invoice"></i><b> Crear Solicitud</b>
							</a>
						@endif
					</div>
