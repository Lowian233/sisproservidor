...
@if(Route::currentRouteName()=='home')
	{{-- fullcalendar --}}
	<script type="text/javascript" src="{{ mix('/js/chart.js') }}"></script>
@endif
...
<script type="text/javascript">
	function Selects(){
		...
		// Excluir .select-native para que dropdowns con opciones estáticas (etapas, tipo actividad CRM) no se rompan
		$('select:not(.select-native)').select2({
...
<script type="text/javascript">
	...
	@if(Route::currentRouteName() === 'clientes.create' || Route::currentRouteName() === 'contactos.create' || Route::currentRouteName() === 'contactos.edit' || Route::currentRouteName() === 'sclientes.create' ||  Route::currentRouteName() === 'sclientes.edit')
		<script type="text/javascript">
			$(document).ready(function(){
				if({{ json_encode(old('SedeExt2')) }} !== null){
		...
		<script type="text/javascript">
			$(document).ready(function(){
			if({{ json_encode(old('SedeExt1')) }} !== null){
	...
	@if(Route::currentRouteName() === 'generadores.create' || Route::currentRouteName() === 'sgeneradores.create' || Route::currentRouteName() === 'sgeneradores.edit')
		<script type="text/javascript">
			$(document).ready(function(){
				if({{ json_encode(old('GSedeExt1')) }} !== null){
					$('.ext').prop('disabled', false);
				};
			});
		</script>
		<script type="text/javascript">
			$(document).ready(function(){
			if({{ json_encode(old('GSedeExt2')) }} !== null){
					$('.ext2').prop('disabled', false);
				};
			});
		</script>
...
<script>
	$(document).ready(function() {
		/* Mostrar los botones básicos de DataTables a TODOS los usuarios.
		   Anteriormente se añadían botones extra sólo para el rol 'Programador'.
		   Ahora forzamos que al menos 'Columnas Visibles' y 'Excel' estén
		   disponibles independientemente del rol. */
		var botoncito = [{extend: 'colvis', text: 'Columnas Visibles'}, {extend: 'excel', text: 'Excel'}];
		/* Antes de inicializar DataTables: comprobar que cada fila del tbody tenga
		   el mismo número de celdas que el thead. Si hay menos, rellenar con
		   celdas vacías para evitar errores en DataTables (p.ej. _DT_CellIndex undefined).
		   Esto evita el fallo que viste en consola para usuarios con columnas condicionales. */
		$('.table').each(function(){
			try{
				var $table = $(this);
				var thCount = $table.find('thead tr').first().children('th').length;
				if(thCount === 0){
					thCount = $table.find('thead').find('th').length;
				}
				$table.find('tbody tr').each(function(){
					var tdCount = $(this).children('td').length;
					if(tdCount < thCount){
						console.warn('DataTable: padding row to match header columns', {thCount: thCount, tdCount: tdCount, table: $table});
				...
				console.error('Error checking table columns before DataTable init', e);
			}
		});
