<div id="tarifa{{$contadorphp}}Container" class="panel panel-default" style="display: inline-block; overflow: hidden; width:100%; background-color:#FAFAFF;">
	...
	<div id="rango{{$contadorphp}}Container" class="col-md-12" style="padding-bottom: 0.25em;">
    	@php
    		$x = 0;
    		// Variable para verificar si puede editar tarifas (Direcci??n T??cnica, Programador, Comercial o JefeOperaciones)
    		$puedeEditarTarifas = isset($esDireccionTecnica) && $esDireccionTecnica || 
    							   in_array(Auth::user()->UsRol, Permisos::COMERCIAL) || 
			...
			@foreach($opcion->tarifas as $tarifa)
				<div class=row>
					<div class="col-md-2 col-xs-6">
						<label data-trigger="hover" data-toggle="popover" data-placement="bottom" title="<b>Tarifa especial</b>" data-content="<p> Activar este switch causara que se tomen en cuenta estas tarifas para definir el precio por unidad durante la conciliaci??n de pesos</p>"> Tarifa Especial
					...
					<div class="col-md-2 col-sm-6" style="max-height: 58.1px;">
						...
						<input {{$puedeEditarTarifas ? '' : 'disabled' }} id="expireRange{{$contadorphp}}" name="Opcion[{{$contadorphp}}][TarifaVencimiento]" type="date" class="form-control" value="{{$tarifa->TarifaVencimiento}}">
						@if(!$puedeEditarTarifas && (in_array(Auth::user()->UsRol, Permisos::JefeOperaciones)||in_array(Auth::user()->UsRol2, Permisos::JefeOperaciones)))
					   		<input type="date" hidden name="Opcion[{{$contadorphp}}][TarifaVencimiento]" value="{{$tarifa->TarifaVencimiento}}">
					    @endif
					</div>
					<div class="col-md-3 col-sm-6">
						<label for="frecrangeSelect{{$contadorphp}}" style="font-size: 0.9em;" data-trigger="hover" data-toggle="popover" title="<b>Frecuencia</b>" data-content="<p> se tomara en cuenta para la aplicaci??n de la tarifa respectiva y el calculo del precio seg??n la frecuencia de la cantidad puesta en planta Prosarc S.A. ESP</p>" >Frec.</label>
						<select {{$puedeEditarTarifas ? '' : 'disabled' }} id="frecrangeSelect{{$contadorphp}}" name="Opcion[{{$contadorphp}}][TarifaFrecuencia]">
					...
					<div class="col-md-3 col-sm-6">
						...
						<select {{$puedeEditarTarifas ? '' : 'disabled' }} id="typerangeSelect{{$contadorphp}}" name="Opcion[{{$contadorphp}}][Tarifatipo]">
					...
					<div class="col-md-2 col-sm-6">
						<label for="addrangeButton{{$contadorphp}}">A?0?9adir Rango</label>
						<button {{$puedeEditarTarifas ? '' : 'disabled' }} style="cursor: cell;" onclick="AgregarRango({{$contadorphp}})" class="btn btn-primary btn-block addrangeButton" id="addrangeButton{{$contadorphp}}"> <i class="fa fa-plus"></i></button>
					</div>
					...
					@if(count($tarifa->rangos) > 0)
						@foreach($tarifa->rangos as $rango)
							...
							<div class="col-md-3 col-sm-6" {{-- {{$last > 1 ? "pull-left": "pull-right"}}" --}} id="rango{{$contadorphp}}{{$last}}">
								...
								@if(($rango->TarifaDesde != 0)&&$puedeEditarTarifas)
