...
<div id="tratamiento{{$contadorphp}}Container" class="panel panel-default" style="display: inline-block; overflow: hidden; width:100%; background-color:#FAFAFF;">
    ...
    <div class="col-md-8" style="margin-bottom: 0.25em;">
        ...
        @endphp
        <select {{$puedeEditar ? '' : 'disabled' }} class="selecttrat" id="opciontratamiento{{$contadorphp}}" name="Opcion[{{$contadorphp}}][Tratamiento]" style="width:100%;">
            ...
            <optgroup label="--------------Viables--------------">
                @foreach($tratamientosViables as $tratamientoviable)
                    @foreach($tratamientoviable->tratamientosConGestor as $tratamientoviable)
                        <!-- <option onclick="recargarAjaxTratamiento/* ({{$contadorphp}}) */ value="{{$tratamientoviable->ID_Trat}}">{{"$tratamientoviable->TratName"}} - {{$tratamientoviable->CliShortname ?? $tratamientoviable->CliName}}</option>
                     -->@endforeach
                @endforeach
            </optgroup>
            <optgroup label="----------Prosarc S.A. ESP----------">
                @foreach($tratamientos as $tratamiento)
                    ...
                    <option  {{ $opcion->FK_ReqTrata === $tratamiento->ID_Trat ? "selected" : "" }} onclick="recargarAjaxTratamiento({{$contadorphp}})" value="{{$tratamiento->ID_Trat}}">{{"$tratamiento->TratName"}} - {{$tratamiento->CliShortname ?? $tratamiento->CliName}}</option>
                    @endif
                @endforeach
            </optgroup>
    ...
    <div class="col-md-2">
        <div class="col-md-12">
            ...
            <input {{$puedeEditar ? '' : 'disabled' }} {{$opcion['ofertado'] == 1 ? "checked=true" : ""}} type="radio" class="ofertaswitch" id="ofert{{$contadorphp}}" name="TratOfertado" value="{{$contadorphp}}"/>
    ...
    <div class="col-md-2">
        <div class="col-md-12">
            ...
            <button class="btn btn-danger droOptionButton" {{$puedeEditar && $OpcionOfertada==0 ? '' : 'disabled' }} onclick="EliminarOption({{$contadorphp}})" id="droOptionButton{{$contadorphp}}"><i class="fas fa-trash"></i></button> 
        </div>
    </div>
</div>