<div id="tratamiento`+contador+`Container" class="panel panel-default" style="display: inline-block; overflow: hidden; width:100%; background-color:#FAFAFF;">
    <div class="col-md-8" style="margin-bottom: 0.25em;">
        <label data-trigger="hover" data-toggle="popover" title="Seleccione Un Tratamiento" data-content="<p> Seleccione entre los tratamientos Viables(segun la Clasificaci??n del residuo) o cualquiera de los tratamientos previamente registrados en la aplicacion SiReS</b></p>" for="tratamiento`+contador+`">
        Tratamiento
        </label>
        <select class="selecttrat" id="opciontratamiento`+contador+`" name="Opcion[`+contador+`][Tratamiento]" style="width:100%;">
            ...
            <optgroup label="--------------Viables--------------">
                @foreach($tratamientosViables as $tratamientoviable)
                    @foreach($tratamientoviable->tratamientosConGestor as $tratamientoviable)
                        <option onclick="recargarAjaxTratamiento(`+contador+`)" value="{{$tratamientoviable->ID_Trat}}">{{"$tratamientoviable->TratName"}} - {{$tratamientoviable->CliShortname ?? $tratamientoviable->CliName}}</option>
                    @endforeach
                @endforeach
            </optgroup>
            <optgroup label="----------Prosarc S.A. ESP----------">
                @foreach($tratamientos as $tratamiento)
                    ...
                    <option  onclick="recargarAjaxTratamiento(`+contador+`)" value="{{$tratamiento->ID_Trat}}">{{"$tratamiento->TratName"}} - {{$tratamiento->CliShortname ?? $tratamiento->CliName}}</option>
                    @endif
                @endforeach
            </optgroup>
    ...
    <div class="col-md-2">
        <div class="col-md-12">
            <label data-trigger="hover" data-toggle="popover" title="Tratamiento Ofertado</b>" data-content="<p> autorizaci??n para que el cliente pueda elegir el tratamiento de este residuo al momento de realizar la solicitud de servicio</p>">  Ofertado</label>
