						...
						<table class="table table-bordered table-striped">
							...
							<tbody id="readyTable">
								@foreach($PublicRespels as $respel)
									@if((in_array(Auth::user()->UsRol, Permisos::PROGRAMADOR)&&($respel->RespelStatus == "Aprobado" || $respel->RespelStatus == "Revisado")||$respel->RespelStatus == "Evaluado"||$respel->RespelStatus == "Cotizado")||in_array(Auth::user()->UsRol, Permisos::TODOPROSARC))
									...
									@if(in_array(Auth::user()->UsRol, ['Programador', 'DireccionTecnica']) || in_array(Auth::user()->UsRol2, ['Programador', 'DireccionTecnica']))
										<td class="text-center">
											@if($respel->RespelStatus == 'Pendiente' || $respel->RespelStatus == 'Incompleto' || $respel->RespelStatus == 'Rechazado')
												<button type="button" class="btn btn-danger btn-sm" onclick="confirmDeletePublic('{{$respel->ID_Respel}}', '{{$respel->RespelName}}')" title="Eliminar residuo público">
...
<!-- Modal de Confirmación para Eliminar Residuo Público -->
<div class="modal fade" id="confirmDeleteModalPublic" tabindex="-1" role="dialog">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h4 class="modal-title">
					<i class="fas fa-exclamation-triangle text-warning"></i> 
					Confirmar Eliminación de Residuo Público
				</h4>
			...
			<div class="modal-body">
				<p><strong>¿Está seguro que desea eliminar el siguiente residuo público?</strong></p>
				<div class="alert alert-warning">
					<i class="fas fa-exclamation-triangle"></i>
					<strong>Residuo:</strong> <span id="residuoPublicName"></span>
				</div>
				<p class="text-muted">
					<small>Esta acción marcará el residuo público como eliminado y no será visible en las listas principales.</small>
				</p>
			</div>
			<div class="modal-footer">
				...
				<form id="deletePublicForm" method="POST" style="display: inline;">
					@csrf
					@method('DELETE')
					<button type="submit" class="btn btn-danger">
						<i class="fas fa-trash"></i> 
						<span id="btn-text">Eliminar Residuo Público</span>
...
@section('NewScript')
<script>
function confirmDeletePublic(residuoId, residuoName) {
	// Actualizar el modal con la información del residuo
	document.getElementById('residuoPublicName').textContent = residuoName;
	
	// Actualizar la acción del formulario
	document.getElementById('deletePublicForm').action = '/respelspublic/' + residuoId;
	
	// Mostrar el modal
	$('#confirmDeleteModalPublic').modal('show');
}

// Manejar el envío del formulario de eliminación
document.getElementById('deletePublicForm').addEventListener('submit', function() {
	// Mostrar spinner y cambiar texto
	document.getElementById('spinner').style.display = 'inline-block';
	document.getElementById('btn-text').textContent = 'Eliminando...';
