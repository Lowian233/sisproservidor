		...
		<div class="col-md-16 col-md-offset-0">
			...
			<div class="box">
				...
				<div class="box box-info">
					<div class="box-body">
						{{-- Filtros (igual que residuos regulares) --}}
						<form method="GET" action="{{ url('respelsexpress') }}" class="form-inline well well-sm" style="margin-bottom: 15px;">
							...
							<a href="{{ url('respelsexpress') }}?buscar=1" class="btn btn-default btn-sm" style="margin-left: 5px;" title="Ver todos los residuos Express (puede tardar más)"><i class="fas fa-list"></i> Ver todos</a>
							@if(request()->hasAny(['nombre','cliente','anio','ver','buscar']))
								<a href="{{ url('respelsexpress') }}" class="btn btn-default btn-sm" style="margin-left: 5px;"><i class="fas fa-times"></i> Limpiar</a>
							@endif
						</form>
						@if(!request()->hasAny(['nombre','cliente','anio','ver','buscar']) && $Respels->isEmpty())
							<div class="alert alert-info" style="margin-bottom: 15px;">
								<i class="fas fa-info-circle"></i>
								<strong>Búsqueda rápida:</strong> Seleccione <b>nombre</b>, <b>cliente</b> y/o <b>año</b> y haga clic en <b>Buscar</b>. O deje <b>Todos</b> y haga clic en <b>Buscar</b> para ver todos los residuos Express.
							</div>
						@endif
						<table class="table table-bordered table-striped" id="respelsExpressTable">
							<thead>
								<tr>
									<th>Creado</th>
									<th>Actualizado</th>
									...
									@if(in_array(Auth::user()->UsRol, ['Programador', 'DireccionTecnica']) || in_array(Auth::user()->UsRol2, ['Programador', 'DireccionTecnica']))
										<td class="text-center">
											@if($respel->RespelStatus == 'Pendiente' || $respel->RespelStatus == 'Incompleto' || $respel->RespelStatus == 'Rechazado')
												<button type="button" class="btn btn-danger btn-sm" onclick="confirmDeleteExpressResidue('{{$respel->RespelSlug}}', '{{$respel->RespelName}}')" title="Eliminar residuo express">
...
<!-- Modal de Confirmación para Eliminar Residuo Express -->
<div class="modal fade" id="confirmDeleteModalExpressResidue" tabindex="-1" role="dialog">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h4 class="modal-title">
					<i class="fas fa-exclamation-triangle text-warning"></i> 
					Confirmar Eliminación de Residuo Express
				</h4>
			...
			<div class="modal-body">
				<p><strong>¿Está seguro que desea eliminar el siguiente residuo express?</strong></p>
				<div class="alert alert-warning">
					<i class="fas fa-exclamation-triangle"></i>
					<strong>Residuo:</strong> <span id="residuoExpressName"></span>
				</div>
				<p class="text-muted">
					<small>Esta acción marcará el residuo express como eliminado y no será visible en las listas principales.</small>
				</p>
			</div>
			<div class="modal-footer">
				...
				<form id="deleteExpressResidueForm" method="POST" style="display: inline;">
					@csrf
					@method('DELETE')
					<button type="submit" class="btn btn-danger">
						<i class="fas fa-trash"></i> 
						<span id="btn-text-express-residue">Eliminar Residuo Express</span>
						<i class="fas fa-spinner fa-spin" id="spinner-express-residue" style="display: none;"></i>
					</button>
				</form>
			</div>
		</div>
...
$(document).ready(function() {
	if ($.fn.DataTable && $.fn.DataTable.isDataTable('#respelsExpressTable')) {
		$('#respelsExpressTable').DataTable().destroy();
	}
	$('#respelsExpressTable').DataTable({
...
function confirmDeleteExpressResidue(residuoSlug, residuoName) {
	document.getElementById('residuoExpressName').textContent = residuoName;
	document.getElementById('deleteExpressResidueForm').action = '/respels/' + residuoSlug;
	$('#confirmDeleteModalExpressResidue').modal('show');
}

document.getElementById('deleteExpressResidueForm').addEventListener('submit', function() {
	document.getElementById('spinner-express-residue').style.display = 'inline-block';
	document.getElementById('btn-text-express-residue').textContent = 'Eliminando...';
