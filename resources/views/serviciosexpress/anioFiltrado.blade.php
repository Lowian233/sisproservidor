...
{{ __('adminlte::message.solsertitle') }} Express {{ $anio }}
@endsection
        ...
        <div class="col-md-16 col-md-offset-0">
            <div class="box">
                <div class="box-header">
                    <h3 class="box-title">{{ __('adminlte::message.solsertitleindex') }} Express - {{ $anio }}</h3>
                    <a href="{{ route('serviciosexpress.index') }}" class="btn btn-default btn-sm pull-right" style="margin-left: 5px;" title="Volver al selector de años"><i class="fas fa-arrow-left"></i> Volver</a>
                    @if(in_array(Auth::user()->UsRol, Permisos::COMERCIALEXPRESS) || in_array(Auth::user()->UsRol2, Permisos::COMERCIALEXPRESS))
                        <a href="{{ route('serviciosexpress.create') }}" class="btn btn-primary pull-right">{{ __('adminlte::message.create') }}</a>
                    @endif
                </div>
                <div class="box box-info">
                    <div class="box-body">
                        {{-- Filtros: mes y cliente dentro del año {{ $anio }} --}}
                        <form method="GET" action="{{ route('serviciosexpress.' . $anio) }}" class="form-inline well well-sm" style="margin-bottom: 15px;">
                            ...
                            <a href="{{ route('serviciosexpress.' . $anio) }}?ver=todos" class="btn btn-default btn-sm" style="margin-left: 5px;" title="Ver todos los servicios Express de {{ $anio }} (puede tardar más)"><i class="fas fa-list"></i> Ver todos</a>
                            @if(request()->hasAny(['mes','cliente','ver','buscar']))
                                <a href="{{ route('serviciosexpress.' . $anio) }}" class="btn btn-default btn-sm" style="margin-left: 5px;"><i class="fas fa-times"></i> Limpiar</a>
                            @endif
                        </form>
                        @if(!request()->hasAny(['mes','cliente','ver','buscar']) && $Servicios->isEmpty())
                            <div class="alert alert-info" style="margin-bottom: 15px;">
                                <i class="fas fa-info-circle"></i>
                                <strong>Búsqueda rápida:</strong> Seleccione <b>mes</b> y/o <b>cliente</b> y haga clic en <b>Buscar</b> para cargar los servicios Express de {{ $anio }}. O use <b>Ver todos</b> para cargar todos los servicios del año (puede tardar más).
                            </div>
                        @endif
                            ...
                            <tbody>
                                ...
                                <tr style="{{$Servicio->SolSerDelete == 1 ? 'color: red' : ''}}">
                                    ...
                                    <td>
                                        ...
                                        @if (isset($Servicio->FK_SedeMun) && $Servicio->FK_SedeMun == 169)
                                        <br>
                                        Localidad: <b>{{$Servicio->SedeMapLocalidad ?? ''}}</b>
                                        @endif
                                    </td>
                                    <td style="text-align: center;"><a href='/serviciosexpress/{{$Servicio->SolSerSlug}}' class="btn btn-info" title="{{ __('adminlte::message.seemoredetails')}}"><i class="fas fa-search"></i></a>
                                    </td>
                                    @if(in_array(Auth::user()->UsRol, Permisos::COMERCIALES) || in_array(Auth::user()->UsRol2, Permisos::COMERCIALES))
                                    @php
