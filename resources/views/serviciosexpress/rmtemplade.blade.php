            ...
            <div align=center>
                ...
                <table width="100%" class="MsoNormalTable" border="1" cellspacing="0" cellpadding="0" style="border-collapse:collapse;border:none;">
                    <tr style='height:26.85pt'>
                        ...
                        @else
                            <td style='border-top:solid windowtext 1.0pt; border-left:none; border-bottom:none;border-right:none; padding:0cm 5.4pt 0cm 5.4pt;height:26.85pt'>
                                <p class=MsoNormal style='margin-bottom:0cm;line-height:normal'><span style='font-size:10.0pt;font-family:"Arial",sans-serif'>{{ $Cliente->CliName ?? 'N/A' }}/
                                <br>
                                {{ $GenerResiduo->GenerName ?? 'N/A' }}</span></p>
                            </td>
                        @endif
                        <td style='border-top:solid windowtext 1.0pt; border-left:none; border-bottom:none;border-right:none; padding:0cm 5.4pt 0cm 5.4pt;height:26.85pt'>
                ...
                <table width="100%" class=MsoNormalTable border=1 cellspacing=0 cellpadding=0 style='border-collapse:collapse;border:none'>
                    ...
                    <tr width="100%" style='height:19.5pt'>
                        ...
                        <td style='border:solid windowtext 1.0pt; border-top:none;padding:0cm 1.4pt 0cm 1.4pt;height:22.8pt'>
                            ...
                            <p class=MsoNormal align=center style='margin-bottom:0cm;text-align:center; line-height:normal'><b><span style='font-size:7.0pt;font-family:"Arial",sans-serif'>{{number_format($totales['conciliado'], $decimals = 2, $dec_point = ',', $thousands_sep = '.')}} kg</span></b></p>
                        </td>
                    ...
                    <tr width=100% style='height:19.5pt'>
                        <td width=10% nowrap colspan=1 style='width:10pt;border:solid windowtext 1.0pt; border-left:solid windowtext 1.0pt;padding:0cm 3.5pt 0cm 3.5pt;height:19.5pt'>
                            <p class="MsoNormal" style="text-align:center;">
                                <span lang="ES">
                                    @if(isset($firmas->FirmaCliente) && !empty($firmas->FirmaCliente))
                                        @php
                                            $rutaFirmaStorage = storage_path('app/public/FirmasClientesExpress/' . $firmas->FirmaCliente . '.png');
                                            $rutaFirmaPublic = public_path('storage/FirmasClientesExpress/' . $firmas->FirmaCliente . '.png');
                                            $rutaFirmaPublicLegacy = public_path('img/FirmasClientesExpress/' . $firmas->FirmaCliente . '.png');
                                            $firmaSrc = null;

                                            if (file_exists($rutaFirmaStorage)) {
                                                $firmaSrc = 'data:image/png;base64,' . base64_encode(file_get_contents($rutaFirmaStorage));
                                            } elseif (file_exists($rutaFirmaPublic)) {
                                                $firmaSrc = 'data:image/png;base64,' . base64_encode(file_get_contents($rutaFirmaPublic));
                                            } elseif (file_exists($rutaFirmaPublicLegacy)) {
                                                $firmaSrc = 'data:image/png;base64,' . base64_encode(file_get_contents($rutaFirmaPublicLegacy));
                                            }
                                        @endphp
                                        @if($firmaSrc)
                                            <img width="118" height="76" id="Imagen 6" src="{{ $firmaSrc }}">
                                        @else
                                            <span style="font-size: 8pt; color: #999;">Sin firma</span>
                                        @endif
                                    @else
                                        <span style="font-size: 8pt; color: #999;">Pendiente de firma</span>
                                    @endif
                                </span>
                            </p>
                        </td>
