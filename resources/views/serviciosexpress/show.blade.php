								...
								<div class="col-xs-6">
									...
									@endif
								     ...
								     @if(in_array(Auth::user()->UsRol, Permisos::COMERCIALEXPRESS) || in_array(Auth::user()->UsRol2, Permisos::COMERCIALEXPRESS))
                                        <label style="margin-left: 10px; margin-right:10px" data-placement="auto" data-trigger="hover" data-html="true" data-toggle="popover" data-delay='{"show": 200}' title="<b>Repetir Solicitud de Servicio</b>" data-content="Puede repetir esta solicitud para crear una nueva con los mismos datos" class="pull-left"><a href='#' data-toggle='modal' data-target='#ModalRepeat' class="btn btn-info"><i class="fas fa-copy"></i> Duplicar</a></label>
                                    @endif
								</div>
								<div class="col-xs-6">
										...
										<div class="modal-content">
											...
											<div class="modal-body">
												...
												<div class="form-group col-md-12">
													...
													<textarea onchange="updatecaracteresrepetir()" id="textDescriptionrepetir" rows ="5" style="resize: vertical;" maxlength="4000" class="form-control col-xs-12" name="solserdescript"></textarea>
												</div>
											</div>
...
{{-- Función para actualizar caracteres restantes en el modal de repetir solicitud --}}
<script>
	$(document).ready(function(){
					...
					<div class="modal-content">
						...
						<form action="/serviciosexpress/certificarExpress" method="POST" enctype="multipart/form-data" data-toggle="validator" id="SolSer">
							<div class="modal-header">
                                ...
                                `+('<div class="form-group col-md-12"><label style="color: black; text-align: left;">Fecha de recepción</label><input required type="date" name="solserRecepcionDate" value="{{ date('Y-m-d') }}" class="form-control col-xs-12" id="solserRecepcionDateInput"></div>')+`
								<input type="submit" id="Cambiar`+slug+`" style="display: none;">
								<input type="text" name="solserslug" value="`+slug+`" style="display: none;">
								<input type="text" name="solserstatus" value="`+status+`" style="display: none;">
							</div>
		...
		@case('Programado')
			...
			@if(in_array(Auth::user()->UsRol, Permisos::PROGRAMADOR) || Auth::user()->email == 'logistica@prosarc.com.co')
				...
				@if(in_array(Auth::user()->UsRol, Permisos::CONDUCTOREXPRESS) || Auth::user()->email == 'logistica@prosarc.com.co')
				 $('#titulo').append(` 
					<a href='{{ route('serviceexpress.recibomaterial', $SolicitudServicio->SolSerSlug) }}' class="btn btn-primary" style="margin: 0 1em;">Recibo Material</a> `); 
				@endif
				@if(in_array(Auth::user()->UsRol, Permisos::CONDUCTOREXPRESS) || Auth::user()->email == 'logistica@prosarc.com.co')
                $('#titulo').append(`
                    <a href='/serviciosexpress/{{$SolicitudServicio->SolSerSlug}}/add-respel ' style="margin-right:1em;" class="btn btn-warning pull-right"><i class="fas fa-exclamation-triangle"></i> Añadir Residuo</a>
