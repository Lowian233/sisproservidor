					...
					<div class="col-md-12 ">
						...
						<div class="box box-info">
                            ...
                            @if(in_array(Auth::user()->UsRol, Permisos::TODOPROSARC) || in_array(Auth::user()->UsRol, Permisos::CLIENTE))
                                @if($foto)
                                    <a href="{{ route('fotos-cliente.show', ['solserslug' => $SolicitudServicio->SolSerSlug]) }}"
                                        ...
                                        data-html="true"
                                        data-delay='{"show": 200}'
                                        title="<b>Fotos Adicionales</b>"
                                        data-content="Haga click para ver las fotos adicionales de Descargue/Pesaje relacionadas con esta Solicitud de Servicio">
                                            <i class="fas fa-images"></i> <b>Fotos Adicionales</b>
                                    </a>
                                @endif
                             @endif
						  ...
						  @if(in_array(Auth::user()->UsRol, Permisos::CLIENTE) || in_array(Auth::user()->UsRol, Permisos::ProgVehic1)|| in_array(Auth::user()->UsRol, Permisos::PROGRAMADOR)||in_array(Auth::user()->UsRol, Permisos::COMERCIALES) )
                            ...
                            @if(in_array(Auth::user()->UsRol, Permisos::SEDECOMERCIAL) || in_array(Auth::user()->UsRol, Permisos::PROGRAMADOR))
												...
												@else
													...
													<td style="text-align: center;">
														@if(in_array(Auth::user()->UsRol, Permisos::ProgVehic2) || in_array(Auth::user()->UsRol2, Permisos::ProgVehic2))
															@if($SolicitudServicio->SolSerStatus === 'Recepcionado' || $SolicitudServicio->SolSerStatus === 'No Conciliado' || $SolicitudServicio->SolSerStatus === 'Corregido')
																@if($Residuo->SolResTypeUnidad == 'Litros' || $Residuo->SolResTypeUnidad == 'Unidad')
...
@php
    ...
    $fechaSolicitud = \Carbon\Carbon::parse($SolicitudServicio->recepcion);
@endphp
@endsection
@section('NewScript')
<script>
...
<script>
	...
	@if($fechaSolicitud->lessThan($mesAnterior))
		@if(in_array(Auth::user()->UsRol, Permisos::PROGRAMADOR) || in_array(Auth::user()->UsRol, Permisos::PROGRAMADOR)||in_array(Auth::user()->UsRol, Permisos::ADMINISTRADORPLANTA) || in_array(Auth::user()->UsRol, Permisos::ADMINISTRADORPLANTA))
		@switch($SolicitudServicio->SolSerStatus)
				@case('Cancelado')
					$('#titulo').empty();
					$('#titulo').append(`
				...
				@case('Programado')
					$('#titulo').append(`
						<b>{{__('adminlte::message.solsershowrecibido')}}</b>
					`);
					$('#titulo').empty();
					@if(in_array(Auth::user()->UsRol, Permisos::PROGRAMADOR))
						$('#titulo').append(`
					...
					@if(in_array(Auth::user()->UsRol, Permisos::SolSer1) || in_array(Auth::user()->UsRol2, Permisos::SolSer1))
						...
						@elseif($ProgramacionesActivas == 0)
							...
							@endif
						@else
							/* eliminado botón Recibida (faltan vehículos) del show */
							$('#titulo').append(`
								<a href='#' onclick="ModalStatus('{{$SolicitudServicio->SolSerSlug}}', 'Residuo Faltante')" style="margin-right:1em;" class="btn btn-warning pull-right"><i class="fas fa-exclamation-triangle"></i> Residuo Faltante</a>
							`);
						@endif
					...
					$('#titulo').append(`
						...
						$('#titulo').append(`
							...
							`);
					@endif
					@if(in_array(Auth::user()->UsRol, Permisos::PROGRAMADOR) || in_array(Auth::user()->UsRol, Permisos::RECEPCIONPDA))
						$('#titulo').append(`
							<a href="/solicitud-servicio/{{$SolicitudServicio->SolSerSlug}}/edit" class="btn btn-warning pull-right"><i class="fas fa-edit"></i><b> {{__('adminlte::message.edit')}}</b></a>
						`);
					@endif
				...
				@case('Completado')
					$('#titulo').empty();
					@if(in_array(Auth::user()->UsRol, Permisos::PROGRAMADOR) || in_array(Auth::user()->UsRol, Permisos::ADMINISTRADORPLANTA))
					$('#titulo').append(`
						<div class="btn-group" style="float: left;">
							<button type="button" style="margin-right:1em;" class="btn btn-danger dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
								Reversar <span class="caret"></span>
					...
					@if(in_array(Auth::user()->UsRol, Permisos::PROGRAMADOR))
						...
						`);
					@endif
					@if(in_array(Auth::user()->UsRol, Permisos::RECEPCIONPDA) || in_array(Auth::user()->UsRol, Permisos::RECEPCIONPDA))
					$('#titulo').append(`
						<a href='#' onclick="ModalStatus('{{$SolicitudServicio->SolSerSlug}}', 'Recepcionado')" style="float: right;" class="btn btn-success"><i class="fas fa-clipboard-check"></i>Recepcionado</a>
					`);
					@endif
					$('#titulo').append(`
						<b>{{__('adminlte::message.solsershowrecibido')}}</b>
				...
				@case('No Conciliado')
					...
					@if(in_array(Auth::user()->UsRol, Permisos::PROGRAMADOR))
						...
						`);
					@endif
					@if(in_array(Auth::user()->UsRol, Permisos::PROGRAMADOR) || in_array(Auth::user()->UsRol, Permisos::ADMINISTRADORPLANTA) || in_array(Auth::user()->UsRol, Permisos::JEFELOGISTICA)|| Auth::user()->email == 'logistica@prosarc.com.co')
						$('#titulo').append(`
							<div class="btn-group" style="float: left;">
								<button type="button" style="margin-right:1em;" class="btn btn-danger dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
									Reversar <span class="caret"></span>
				...
				@case('Tratado')
					$('#titulo').empty();
					@if(in_array(Auth::user()->UsRol, Permisos::PROGRAMADOR) || in_array(Auth::user()->UsRol, Permisos::ADMINISTRADORPLANTA)|| in_array(Auth::user()->UsRol, Permisos::JEFELOGISTICA))
					$('#titulo').append(`
					<div class="btn-group" style="float: left;">
						<button type="button" style="margin-right:1em;" class="btn btn-danger dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
							Reversar <span class="caret"></span>
				...
				@case('Certificacion')
					$('#titulo').empty();
					@if(in_array(Auth::user()->UsRol, Permisos::REVERSARADMIN) || in_array(Auth::user()->UsRol2, Permisos::REVERSARADMIN) || in_array(Auth::user()->UsRol, Permisos::JEFELOGISTICA))
						$('#titulo').append(`
							<div class="btn-group" style="float: left;">
								<button type="button" style="margin-right:1em;" class="btn btn-danger dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
									Reversar <span class="caret"></span>
				...
				$('#titulo').empty();
					...
					@if(in_array(Auth::user()->UsRol, Permisos::CLIENTE) || in_array(Auth::user()->UsRol, Permisos::AREALOGISTICA))
						...
						$('#titulo').append(`
							...
							`);
					@endif
					@if(in_array(Auth::user()->UsRol, Permisos::USAQUEN) && $ultimoRecordatorio->ObsRepeat > 3)
						$('#titulo').append(`
							<a href='#' onclick="ModalStatus('{{$SolicitudServicio->SolSerSlug}}', 'Conciliada')" style="float: right;" class="btn btn-success"><i class="fas fa-clipboard-check"></i> {{__('adminlte::message.solserstatusconciliado')}}</a>
						`);
					@endif
